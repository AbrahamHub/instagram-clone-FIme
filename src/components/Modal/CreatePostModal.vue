...
watch(currentTab, () => {
  switch (currentTab.value) {
    case 'InitPost':
      title.value = 'Crear nuevo artículo'
      break
    case 'EditorPost':
      title.value = 'Cortar'
      break
...
<template>
      ...
      >
        ...
        <UiButton v-if="isShowButton" variant="text" @click="handleNextTab">
          <span class="text-buttonColor-primary select-none hover:text-link">{{
            currentTab != 'CaptionPost' ? 'Siguiente' : 'Compartir'
          }}</span>
        </UiButton>
      </div>
  ...
  <RemovePopup
    ...
    title="Remover publicaciones?"
    desc="Si sales, perderás lo que acabas de editar."
  ...
  <RemovePopup
    ...
    title="¿Quitar fotos?"
    desc="Esto eliminará la imagen de tu publicación."
