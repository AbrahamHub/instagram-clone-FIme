  ...
  if (files) {
    ...
    for (const index in fileArr) {
      if (fileArr[index].size < 1024 * 5) {
        ...
        setTitle('El archivo debe tener un tamaño de 5 KB o más')
        break
      }
    }
...
<template>
  <div class="w-[400px] h-full p-6" @dragover="handleDragOver($event)" @drop="handleDrop($event)">
    ...
    >
      ...
      <span class="text-xl mt-[10px]">Arrastra fotos y vídeos aquí</span>
      <span v-if="isError" class="text-sm text-textColor-secondary text-center mt-1">
        <span class="font-semibold">{{ fileErrorName }}</span>
      Demasiado pequeña. Para continuar, seleccione un archivo de 5 KB o más
      </span>
      <UiButton primary class="mt-5">
        <span>Seleccionar desde la computadora</span>
        <input
          ref="inputFiles"
          accept="image/*, video/*"
          type="file"
